...
////
function _path<A extends string, B extends string, C extends string, D>
  (path: [A, B, C],
  d: {[K1 in A]: {[K2 in B]: {[K3 in C]: D}}}
  ): D


function _path(path: any, d: any): any {
  let ret = d
  for (let k of path) {
    ret = ret[k]
  }
...
function path<A extends string, B extends string, C extends string, D>
  (path: [A, B, C],
  d: {[K1 in A]: {[K2 in B]: {[K3 in C]: D}}}
  ): D
